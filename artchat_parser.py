...
async def main():
    ...
    for start in range(0, len(groups), CHUNK):
        ...
        # ---- состояние папки Arts на GitHub ----
        base_sha  = gh('GET', f'/git/ref/heads/{BRANCH}')['object']['sha']
        base_tree = gh('GET', f'/git/commits/{base_sha}')['tree']['sha']
        tree_list = gh('GET', f'/git/trees/{base_tree}?recursive=1').get('tree', [])
        sizes = {i['path'][len(PREFIX):]: i['size']
                 for i in tree_list
                 if i['path'].startswith(PREFIX) and i['type'] == 'blob'}
        ...
        # ---- лимит 1.5 ГБ: вытесняем старейшие записи вместе с картинками ----
        new_bytes = sum(os.path.getsize(os.path.join(TRANSIT, f)) for f in files)
        projected = sum(sizes.values()) + new_bytes
        to_delete, idx = [], 0
        while projected > LIMIT_BYTES:
            while idx < len(old) and not any(f in sizes for f in old[idx].get('images', [])):
                idx += 1
            if idx >= len(old):
                print('⚠️ Лимит превышен, но удалять больше нечего.')
                break
            e = old.pop(idx)
            for f in e.get('images', []):
                if f in sizes:
                    projected -= sizes[f]
                    to_delete.append(f)
            print(f'  🗑 вне лимита: seed {e.get("seed")}, файлов {len(e.get("images", []))}')

        # ---- собираем коммит: новые файлы + удаления + обновлённый JSON ----
        tree = []
        for fname in files:
        ...
        for fname in to_delete:
            tree.append({'path': PREFIX + fname, 'mode': '100644',
                         'type': 'blob', 'sha': None})   # None = удалить файл
        ...
        commit = gh('POST', '/git/commits', {
            'message': f'ArtChat sync: +{len(files)} arts, -{len(to_delete)} old',
